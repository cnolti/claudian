...
- **Tool-call grouping** — `src/features/chat/rendering/toolCallGrouping.ts` collapses runs of ≥2 consecutive tool/thinking blocks. Progressive during streaming: trailing run capped at `STREAMING_TRAILING_VISIBLE` newest calls (hook: `StreamController.flushPendingTools`), overflow merges into the adjacent group; final pass in `InputController`'s turn-finally block (NOT `resetStreamingState` — that only runs on tab recycle), replay pass in `MessageRenderer`. Running subagents never group. (A Haiku status narrator was tried and removed: per-narration CLI cold starts cost 6–15s — too slow for live status.)
